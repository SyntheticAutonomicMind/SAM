...
public class MLXProvider: AIProvider {
    ...
    /// Apply LoRA adapter weights to the base model using MLX's native LoRAContainer.
    ///
    /// This method uses MLX Swift LM's built-in LoRA support to apply adapter weights
    /// to the model. The adapter directory must contain:
    /// - `adapter_config.json`: LoRA configuration (rank, scale, num_layers)
    /// - `adapters.safetensors`: LoRA weights in safetensors format
    ///
    /// - Parameter model: The loaded base model
    /// - Returns: Model with LoRA weights applied
